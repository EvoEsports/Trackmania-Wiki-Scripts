"""
GBX remote connector by snixtho
modified for method reference generation
for the Trackmania Wiki.
"""
...
class DedicatedRemote:
    ...
    def __internal_reconnect(self):
        while True:
            ...
            print('Attempting to reconnect callback loop.')

            if self.connect():
                return
            
            print('Failed to connect callback loop, trying again soon ...')
            time.sleep(1)
    ...
    def _authenticate(self):
        ...
        if type(success) is xmlrpc.client.Fault:
            print('Authentication failed: %s' % str(success.faultString))
            return False
    ...
    def _attempt_connection(self):
        try:
            ...
            if header not in self.validHeaders:
                print('Remote returned an invalid header: %s' % str(header))
                return False

            return True
        except socket.gaierror:
            print('Failed to resolve host %s.' % str(self.host))
        except socket.error as e:
            print('Connection failed to %s:%d: %s' % (str(self.host), int(self.port), str(e)))
        except Exception as e:
            print('Connection failed due to unknown error: %s' % str(e), exc_info=e)
    ...
    def _handle_result(self, handler, method, data):
        ...
        try:
            ...
            print('Failed handling callback: %s' % str(e), exc_info=e)
    ...
    def _result_loop(self):
        ...
        while True:
            ...
            try:
                ...
                pass
                #print('Failed to recieve.', exc_info=e)
    ...
    def connect(self, maxretries=-1, attemptcb=None):
        ...
        while True:
            ...
            print('TM2 Dedicated Server Connection Failed, re-try %d of %d ...' % (int(retries), int(maxretries)))
    ...
    def call(self, method, *args, retryConnection=True, asynchronous=False):
        ...
        try:
            ...
            print('Connection to TM2 Dedicated server has been lost: %s' % str(e))

            if retryConnection:
                print('Will attempt to re-connect.')
                
                if self.connect(self.connRetries):
                    return self.call(method, *args, retryConnection=False)
                
                print('Re-connection failed.')
                raise e
        
        except Exception as e:
            print('Unknown error during XMLRPC call attempt: %s' % str(e), exc_info=e)
            raise e
